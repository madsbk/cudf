...
{
  // A cross join produces `left.num_rows() * right.num_rows()` rows and
  // `left.num_columns() + right.num_columns()` columns. An operand with zero
  // columns but a non-zero row count is valid and contributes only rows, so we
  // no longer reject empty-column operands here.
  ...
  // Pass the row count explicitly so a zero-column result (both operands empty
  // of columns) still reports the correct height.
